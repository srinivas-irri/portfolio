      ...
      <div className='flex gap-2 experience-section max-[640px]:gap-3'>
          ...
          <div className='w-11/12'>
           {experience.map((job, index) => (
                 <div className={`exp-section mb-12 relative after:w-0.5 after:h-12/12 after:bg-gray-700 after:content after:absolute after:top-8 after:-left-8 before:w-0.5 before:h-2/12 before:bg-gray-700 before:content before:absolute before:-top-5 before:-left-8 max-[640px]:after:-left-7 max-[640px]:before:-left-7 max-[640px]:last:before:h-1/12 ${job.serialNo === '1' ? 'before:hidden' : ''} ${job.serialNo === '4' ? 'after:hidden' : ''}`} key={index}>
                <div className={`mb-3 text-left relative before:content before:w-4 before:h-4 before:border-1 before:border-teal-500 before:bg-gray-950 before:rounded-lg before:absolute before:top-6/12 before:-translate-y-6/12 before:-left-10 before:z-20 job-info max-[640px]:before:-left-9  ${job.serialNo === '1' ? 'before:bg-teal-500' : ''}`}>
                    <div className='text-sm mb-1 text-[#61c0ff] duration'>{job.year.from} - {job.year.to}</div>
                ...
                <div className='mt-4 core_skills'>
                    <ul className='text-gray-400 text-xs text-left'>
                      {job.technologies.map((tech, index) => (
                          <li key={index} className='inline-block mr-3 border-1 rounded-4xl py-2 px-3 mb-3 text-gray-400 border-gray-700'>{tech}</li> 
                      ))}
                    </ul>
                </div>
              </div>
